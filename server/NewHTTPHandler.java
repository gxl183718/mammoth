...
public class NewHTTPHandler extends AbstractHandler {
    private ServerConf conf;
    private StorePhoto sp;
    private ClientAPI clientAPI;

    NewHTTPHandler(ServerConf conf) throws Exception {
        this.conf = conf;
        sp = new StorePhoto(conf);
        clientAPI = new ClientAPI();
        clientAPI.init("STL://10.136.134.16:26379;10.136.134.17:26379;10.136.134.18:26379;" +
                "10.136.134.19:26379;10.136.134.20:26379");
...
//        response.setHeader("Content-Length",flen-off+"");
    ...
    private void doGet(Request baseRequest, HttpServletRequest request,
        ...
        int off = 0;
        if (range != null) {
            off = Integer.parseInt(range.split("[=\\-]")[1]);
        }
        if (key != null) {
            try {
                byte[] bytes = clientAPI.get(key);
                if (bytes != null && bytes.length > 0) {
                    int flen = bytes.length;
                    response.setHeader("Content-Range", "bytes " + off + "-" + (flen - 1) + "/" + flen);
                    switch (key.charAt(0)) {
                        case 'i':
                    ...
                    response.getOutputStream().write(bytes, off,flen);
                } else {
                    response.getWriter().print("Object does not exist(" + key + ")");
                }
            } catch (Exception e) {
                try {
...
//				elements = re.getResult().stream().map(Entry::getKey).collect(Collectors.toSet());
    ...
    public static void main(String args[]){
        byte[] bs = {'a', 'b', 'c', 'd', 'e'};
        System.out.println(new String(bs, 3, 2));
    }
